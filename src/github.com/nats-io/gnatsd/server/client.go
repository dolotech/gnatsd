...
type client struct {
	...
	stats					// 消息订阅数量、下发数量、阻塞量等数据记录
	mpay  int64				// 允许的最大消息大小默认1M
	mu    sync.Mutex
	typ   int
	cid   uint64			// 原子自增的客户端唯一标识
	opts  clientOpts
	start time.Time
	nc    net.Conn			// 本客户端数据流网络连接
	ncs   string
	bw    *bufio.Writer
	srv   *Server			// 消息服主对象结构
	...
	flags clientFlag // 每个客户连接的状态掩码标记位  Compact booleans into a single field. Size will be increased when needed.
}
